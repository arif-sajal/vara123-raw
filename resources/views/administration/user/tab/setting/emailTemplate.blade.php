    ...
    <ul class="nav nav-tabs nav-underline no-hover-bg">
        @foreach ($tabs as $tab)
            <li class="nav-item" data-tab-url="{{ route('user.settings',['tab1'=>request()->tab1,'tab2'=>$tab]) }}">
                <a class="nav-link @if((request()->has('tab2') && request()->tab2 == $tab) || (!request()->has('tab2') && $loop->index == 0)) active @endif" data-toggle="tab" data-href="#tabViewOne" data-content="{{ route('user.tab.setting.email.template.single',$tab) }}" aria-controls="active" aria-expanded="true">{{ ucfirst($tab) }}</a>
            </li>
        @endforeach
    </ul>
</div>
