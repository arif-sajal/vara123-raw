            ...
            <div class="collapse navbar-collapse" id="navbar-mobile">
                ...
                <ul class="nav navbar-nav float-right">
                    <li class="dropdown dropdown-user nav-item">
                        ...
                        <span class="mr-1">
                          <span class="user-name text-bold-700">{{ auth()->guard('user')->user()->profile->full_name }}</span>
                        </span>
                            <span class="avatar avatar-online">
                                @if(Storage::has(auth()->guard('user')->user()->profile->image))
                                    <img src="{{ Storage::url(auth()->guard('user')->user()->profile->image) }}" alt="{{ auth()->guard('user')->user()->profile->full_name }}"><i></i>
                                @else
                                    <img src="{{ Storage::url(config('default_user_avatar')) }}" alt="{{ auth()->guard('user')->user()->profile->full_name }}"><i></i>
                                @endif
                            </span>
                        </a>
                    ...
                    @if(SC::get('enable_multi_language') == 'on')
                        <li class="dropdown dropdown-language nav-item">
                            ...
                            <div class="dropdown-menu" aria-labelledby="dropdown-flag">
                                @foreach (\App\Models\Language::where('has',true)->get() as $language)
                                    <a class="dropdown-item" href="#"><i class="flag-icon flag-icon-{{ $language->icon }}"></i> {{ $language->name }}</a>
                                @endforeach
                            </div>
                        </li>
                    @endif
