    ...
    <div role="tabpanel" class="tab-pane active" id="tabView" aria-labelledby="active-tab" aria-expanded="true">
        <div class="row" style="margin-bottom: 15px">
            <div class="col-md-12">
                ...
                @foreach( App\Models\PropertyType::where('id',3)->get() as $property_type )
                <input type="hidden" class="form-control property_id" value="{{ $property_type->id }}">
                @endforeach
            </div>
        </div>
...
<script>
    $(document).ready(function(){
         $(".search").on('input', function(v){
             ...
             $.ajax({
                 type: 'GET',
                 url : '/search/'+value+'/property_type_id/'+id,
                 dataType : "json",
